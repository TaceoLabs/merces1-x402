      ...
      <div className="overflow-x-auto rounded-[0.5rem] border border-zinc-200 bg-white shadow-[0_2px_4px_rgba(0,0,0,0.04)]">
        <table className="w-full min-w-[36rem] border-collapse text-left">
          ...
          <tbody>
            {txsLoading ? (
              <tr>
                <td colSpan={5} className="px-4 py-10 text-sm text-center text-zinc-400">
                  Loading…
                </td>
              </tr>
            ) : pagedTxs.length === 0 ? (
              ...
              pagedTxs.map((tx) => {
                ...
                const txLabel = tx.txHash
                  ? `${tx.txHash.slice(0, 4)}...${tx.txHash.slice(-4)}`
                  : `#${tx.id}`;
                const explorerHref =
                  tx.txHash && blockExplorerUrl ? `${blockExplorerUrl}/tx/${tx.txHash}` : null;
          ...
          <div className="flex items-center gap-1">
            {paginationItems.map((item, index) =>
              item === 'ellipsis' ? (
                <span
                  key={`ellipsis-${index}`}
                  className="inline-flex size-8 items-center justify-center text-sm text-zinc-400"
                >
                  …
                </span>
              ) : (
