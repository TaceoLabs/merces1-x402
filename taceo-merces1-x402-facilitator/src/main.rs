...
use taceo_merces1_x402_facilitator::config::X402FacilitatorServiceConfig;

/// Top-level configuration for the x402 facilitator binary.
///
/// Configured via environment variables using the `MERCES1_X402_FACILITATOR__` prefix
/// and `__` as the separator.
#[derive(Clone, Debug, Deserialize)]
struct X402FacilitatorConfig {
    ...
    /// The facilitator service config.
    #[serde(rename = "service")]
    pub service_config: X402FacilitatorServiceConfig,
}
...
async fn run() -> eyre::Result<()> {
    let config = Config::builder()
        .add_source(
            config::Environment::with_prefix("MERCES1_X402_FACILITATOR")
                .separator("__")
                .list_separator(",")
                .with_list_parse_key("service.node_urls")
                .with_list_parse_key("service.mpc_pks")
                .with_list_parse_key("service.rpc.http_urls")
                .try_parsing(true),
        )
        .build()?
        .try_deserialize::<X402FacilitatorConfig>()?;
    ...
    let router = taceo_merces1_x402_facilitator::start(config.service_config)
        .await
        .context("while initializing x402 facilitator")?;
    ...
    tracing::info!("x402 facilitator started - waiting for shutdown...");
